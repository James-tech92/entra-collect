...
function buildCaExclusionIndex(caAudit) {
  ...
  // The column is a display list: "Albert DUPONT <adupont@tenant> | Svc <…>".
  const byUpn = new Map();
...
/**
 * Enforced MFA CA that covers a broad interactive population (not just a PoC
 * admin). Used to avoid treating raw MFA-registration inventory as Critical.
 */
function detectEnforcedMfaCoverage(caAudit) {
  ...
  for (const r of enforced) {
    ...
    const incl = `${r.IncludeUsers || ""} ${r.IncludeGroups || ""}`;
    const isMfa =
      /mfa|multifactor|authentication strength/i.test(`${name} ${grant}`) &&
      !/\bblock\b/i.test(String(r.GrantControls || ""));
    if (!isMfa) continue;
    const inclUsers = String(r.IncludeUsers || "").trim();
    const inclGroups = String(r.IncludeGroups || "").trim();
    // Empty include on an MFA grant often means "All users" in Graph exports.
    const includeLooksAll =
      /all users/i.test(incl) ||
      (!inclUsers && !inclGroups) ||
      /^all$/i.test(inclUsers);
    const broad =
      includeLooksAll ||
      /for all (internal )?users|all internal users|require multifactor authentication for all|^mfa policy$/i.test(
        name
      ) ||
      /\bMS_MFA\b|licen|everyone|all employees/i.test(incl);
    if (broad) evidence.push(name);
  }
  return { covered: evidence.length > 0, evidence: evidence.slice(0, 6) };
}
...
function looksLikeServiceAccount(upn, displayName) {
  ...
  return (
    /^(svc[-._]|service[-._]|spn[-._]|sync_)/i.test(local) ||
...
function classifyAccountKind(upn, displayName) {
  ...
  if (
    ...
    /^(svc|sa|service)[-_.]|\bservice\b|\bsvc\b/i.test(blob)
  ) {
    return "service";
  }
  const local = String(upn || "").split("@")[0];
...
function runRules(ctx) {
  ...
  // NARR.Guest.Stale, which covers the same population for the right reason.
  {
    ...
    if (exposed.length >= 10 && noMfaSet.size) {
      ...
      // Two designs both count as covered: MFA required of everyone, or the
      // more common pair of MFA scoped to the licensed population plus a block
      // on everyone unlicensed/unmanaged.
      ...
      const allUserMfa = enforcedCa.filter(
        (r) => isMfaPolicy(r) && /all users/i.test(String(r.IncludeUsers || ""))
      );
      // Sort the ones that say MFA in their name first — they read as evidence;
      // a policy that only matched on auth strength does not.
      ...
      const mfaCovered = allUserMfa.length > 0;
      ...
      if (mfaCovered) {
        evidence.push(
          `Enforced all-users MFA policy in place: ${allUserMfa.slice(0, 2).map((r) => r.PolicyName).join(" ; ")} — interactive sign-in on these accounts would be challenged and fail`
        );
      }
      ...
      pushNarrative(narratives, {
        ...
        priority: "Next",
        title: "Dormant accounts with no MFA — mostly shared mailboxes and rooms, not users",
        narrative:
  ...
  // ── NARR.App.RoleManagement — path to assign GA (Critical only if not known backup/SOC tooling) ─
  {
  ...
  // ── NARR.App.PathToGA — other GA-path perms (exclude RoleMgmt + Microsoft 1P) ─
  {
    const rows = (ctx.gaPath.length ? ctx.gaPath : ctx.dangerous).filter(
      (r) =>
        GA_PATH_PERMS.has(r.Permission) &&
        r.Permission !== ROLE_MGMT_PERM &&
        !isMicrosoftFirstPartyApp(
          r.SPNDisplayName || r.DisplayName || r.PrincipalId
        )
    );
    if (rows.length) {
      const byApp = new Map();
      for (const r of rows) {
        const name = r.SPNDisplayName || r.DisplayName || r.PrincipalId;
        if (!byApp.has(name)) byApp.set(name, new Set());
        byApp.get(name).add(r.Permission);
      }
      pushNarrative(narratives, {
        id: "NARR.App.PathToGA",
        severity: "High",
        title: "Non-Microsoft apps with directory write / GA-adjacent Graph permissions",
        narrative:
          `${byApp.size} third-party app(s) have Application.ReadWrite.All, Directory.ReadWrite.All, and/or AppRoleAssignment.ReadWrite.All — often convertible to Global Admin via credential theft and app/role manipulation. ` +
          "Microsoft first-party apps (e.g. Microsoft 365 Backup) are excluded here — they still need credential hygiene, but they are not vendor consent sprawl. " +
          "Third-party grants are usually accepted wholesale from a consent screen, which asks for everything the product can do rather than what this deployment uses.",
        evidence: [...byApp.entries()]
          .map(([n, perms]) => `${n} [${[...perms].join(", ")}]`)
          .join(" | "),
        remediation:
  ...
  // ── NARR.CA.SecInfoReg ─
  {
    if (
      secInfoCa === false ||
      /fail/i.test(checklistStatus(ctx.checklist, "AP.CA.SecInfoReg"))
    ) {
      pushNarrative(narratives, {
  ...
  // ── NARR.CA.ReportOnlyCompliance ─
  {
    ...
    const deviceComplianceReport = reportOnly.filter((r) =>
      /compliant|enrolled|Security Info Registration/i.test(
        `${policyLabel(r)} ${r.GrantControls || ""} ${r.RiskFlags || ""}`
      )
    );
    const findingHint = (ctx.rawFindings || []).find((f) =>
      /report-only.*device|device-compliance CA still report-only/i.test(
        f.Detail || ""
      )
    );
    ...
    if (hardeningReport.length) {
      pushNarrative(narratives, {
        ...
        remediation:
          "Validate impact in report-only, then enforce Token Protection on Windows EXO/SPO clients and phishing-resistant MFA for users (admins already enforced where CA43 is On).",
  ...
  // ── NARR.MFA.MassGap — context-aware when CA blocks unlicensed / scopes MFA to licenses ─
  {
    ...
    if (n >= 50 || adminsReported >= 1 || humanAdminCount >= 1) {
      ...
      if (humanAdminCount >= 1 && !compensated) {
        ...
        severity = "High";
        priority = "Now";
        narrative = `${humanAdminCount} human admin account(s) lack MFA registration. Broader tenant gap is ${n} users without MFA methods, but Conditional Access already requires MFA for the interactive population (${(mfaCa.evidence.length ? mfaCa.evidence : gate.evidence).slice(0, 2).join(", ") || "see 02_CA_Audit.csv"}) — prioritize closing the admin registration gap and reviewing MFA exclusion groups (MS_NO_MFA*).`;
      } else if (compensated) {
        severity = "Medium";
        priority = "Next";
        narrative =
          ...
          "Accounts without methods are challenged and fail interactive MFA, unless they sit in an exclusion group (MS_NO_MFA*, travel exemptions) or use legacy/basic auth. " +
      ...
      pushNarrative(narratives, {
        ...
        evidence:
          ...
          (mfaCa.covered
            ? `; enforcedMfaCA=true (${mfaCa.evidence.join(" | ")})`
            : "; enforcedMfaCA=false") +
          " — 07_Users_Without_MFA.csv",
        remediation: compensated
          ? "Do not run a blanket MFA campaign on the raw count. Audit MS_NO_MFA* / travel exclusion group membership (make those groups role-assignable), confirm legacy auth is blocked, register MFA for any remaining licensed interactive humans, and treat shared/service objects as sign-in-blocked rather than MFA targets."
  ...
  // undifferentiated list into "harden this now" vs "delete these credentials".
  {
    ...
    /** @type {Map<string, {perms:Set<string>, sev:Set<string>}>} */
    const risky = new Map();
    for (const r of dangerous) {
      const name = r.SPNDisplayName || r.DisplayName || r.AppName;
      if (!name) continue;
      if (!risky.has(name)) risky.set(name, { perms: new Set(), sev: new Set() });
      const e = risky.get(name);
      if (r.Permission) e.perms.add(r.Permission);
      if (r.Severity) e.sev.add(r.Severity);
    }

    /** @type {Map<string, {n:number, ips:Set<string>, locs:Set<string>, failures:number}>} */
    const activity = new Map();
    for (const r of signIns) {
      const name = r.App || r.AppDisplayName;
      if (!name) continue;
      if (!activity.has(name))
        activity.set(name, { n: 0, ips: new Set(), locs: new Set(), failures: 0 });
      const e = activity.get(name);
      e.n++;
      if (r.IP) e.ips.add(r.IP);
      if (r.Location) e.locs.add(r.Location);
      // Entra encodes success as error code 0.
      if (r.Status && String(r.Status).trim() !== "0") e.failures++;
    }

    const active = [...risky.keys()]
      .filter((n) => activity.has(n))
      .map((n) => ({ name: n, ...activity.get(n), ...risky.get(n) }))
      .sort((a, b) => b.n - a.n);
    ...
    const digestHot = (ctx.spnSignInDigest || []).some(
      (r) =>
        /yes/i.test(String(r.HighPrivSpn || "")) && Number(r.Events || 0) > 0
    );
    if (signIns.length && active.length && !digestHot) {
    ...
    const dormant = [...risky.keys()].filter((n) => !activity.has(n));
    if (signIns.length && dormant.length >= 3) {
      const critical = dormant.filter((n) => risky.get(n).sev.has("Critical"));
      pushNarrative(narratives, {
        ...
        evidence: dormant
          .slice(0, 10)
          .map((n) => `${n} [${[...risky.get(n).perms].slice(0, 2).join(", ")}]`)
          .join(" | "),
        remediation:
  ...
  // ── NARR.Endpoint.TvmCves — unpatched High/Critical CVEs on devices ─
  {
    ...
    if (vulns.length || tvmWin.length) {
      const crit = vulns.filter((r) =>
        /critical/i.test(
          String(r.VulnerabilitySeverityLevel || r.Severity || "")
        )
      ).length;
      const devices = new Set(
        [
          ...vulns.map((r) => r.DeviceId || r.DeviceName).filter(Boolean),
          ...tvmWin.map((r) => r.DeviceId || r.DeviceName).filter(Boolean),
        ]
      ).size;
      const sampleVulns = vulns
        .slice(0, 6)
        .map(
          (r) =>
      ...
      const sampleWin = tvmWin
        ...
        .join(" | ");
      const hasCritOs = tvmWin.some((r) => Number(r.Critical) > 0);
      pushNarrative(narratives, {
        id: "NARR.Endpoint.TvmCves",
        severity: crit || hasCritOs ? "High" : "Medium",
        priority: "Now",
        title:
          crit || hasCritOs
            ? "Unpatched Critical/High CVEs on Defender-managed devices"
            : "Unpatched High-severity CVEs on Defender-managed devices",
        narrative:
          `Defender Threat & Vulnerability Management shows unpatched High/Critical software on ~${devices || "n/a"} device(s)` +
          (crit ? ` — including ${crit} Critical CVE row(s)` : "") +
          (tvmWin.length
            ? ` and ${tvmWin.length} Windows OS device row(s) with High/Critical OS vulns`
            : "") +
          (vulns.length ? ` (${vulns.length} CVE×software inventory row(s) in 11_*).` : ".") +
          " Unpatched OS CVEs are a primary ransomware / lateral-movement path.",
        evidence:
          [sampleWin, sampleVulns].filter(Boolean).join(" || ") ||
          `${vulns.length} rows in 11_Defender_Exploitable_Vulns.csv`,
        remediation:
          "Prioritize Critical on windows_* in 32_TVM_Windows_HighCritical.csv; enforce update rings; isolate devices that cannot patch.",
        relatedFiles: [
  ...
  // ── NARR.Endpoint.PatchLag — behind Patch Tuesday ─
  {
    ...
    if (behind > 0) {
      pushNarrative(narratives, {
        ...
        narrative: `${behind} device(s) lag the collector Patch Tuesday map (${ctx.summary.patchTuesdayAsOf || "see 30_patch_tuesday_reference.json"}). Delayed cumulative updates leave known CVE windows open.`,
  ...
  // MDE they are often the only hunting-backed activity findings.
  {
    ...
    if (hot.length) {
      pushNarrative(narratives, {
        ...
        severity: hot.some((r) => /critical/i.test(r.Severity || ""))
          ? "High"
          : "Medium",
        priority: "Now",
        title: `${hot.length} High/Critical Defender alert(s) in the last 30 days`,
        narrative:
          `Defender XDR raised ${hot.length} High/Critical alert group(s) (of ${alerts.length} total) in the last 30 days. ` +
          "These are detections the platform already believes matter — treat them as demonstrated activity, not inventory. " +
          ...
          ctx.summary.adaptiveIntel.mdeEndpoint === false
            ...
            : "Correlate with endpoint evidence in the deep dive where MDE is onboarded."),
        evidence: hot
          .slice(0, 10)
          .map(
            (r) =>
...

  {
    ...
    if (rows.length >= 5) {
      const withRisk = rows.filter((r) =>
        /high|medium|low|atRisk|compromised/i.test(String(r.RiskLevel || ""))
      );
      ...
      pushNarrative(narratives, {
        ...
        severity: withRisk.length ? "High" : "Medium",
        priority: "Next",
        title: `Defender IdentityInfo: ${rows.length} critical / risk / role-bearing identities`,
        narrative:
          `Advanced Hunting IdentityInfo (UEBA inventory) lists ${rows.length} identities with elevated criticality, risk, blast radius or directory roles` +
          (withRisk.length ? ` — ${withRisk.length} carry a RiskLevel` : "") +
          (withRoles.length ? `, ${withRoles.length} have AssignedRoles / PIM roles` : "") +
          ". This table often has rows when IdentityLogonEvents is empty, so it is the best Defender-side identity signal on tenants without MDE device streaming.",
        evidence: rows
          .slice(0, 12)
          .map(
            (r) =>
...

  {
    ...
    if (hot.length) {
      const roleMgmtLive = hot.some((r) =>
        /rolemanagement|appelements|avepoint|bpg-operations|approleassignment/i.test(
          `${r.App || ""} ${r.Permissions || ""}`
        )
      );
      ...
      pushNarrative(narratives, {
        ...
        severity: roleMgmtLive || spread.length ? "High" : "Medium",
        priority: roleMgmtLive || spread.length ? "Now" : "Next",
        title: `${hot.length} high-privilege third-party app(s) actively signing in`,
        narrative:
  ...
  {
    const rows = (ctx.failedSignInsByIp || [])
      .map((r) => ({ ...r, n: Number(r.Failures || 0) }))
      .filter((r) => r.n > 0)
      .sort((a, b) => b.n - a.n);
  ...
  {
    const row = (ctx.passwordSettings || []).find(
      (r) =>
        r.LockoutThreshold != null ||
        r.EnableBannedPasswordCheck != null ||
        /Password Rule/i.test(r.DisplayName || "")
    );
    if (row) {
      const bannedOff = /false|0/i.test(String(row.EnableBannedPasswordCheck));
      const listEmpty = !String(row.BannedPasswordList || "").trim();
      const onPremAudit = /audit/i.test(
        String(row.BannedPasswordCheckOnPremisesMode || "")
      );
      pushNarrative(narratives, {
        ...
        narrative:
          `Entra Password Protection (Password Rule Settings) reports smart lockout threshold=${row.LockoutThreshold ?? "?"} lasting ${row.LockoutDurationInSeconds ?? "?"}s. ` +
          (bannedOff
          ...
          (listEmpty && !bannedOff
            ...
            : "") +
          `On-premises password protection is ${row.EnableBannedPasswordCheckOnPremises || "?"} in mode ${row.BannedPasswordCheckOnPremisesMode || "?"} ` +
          (onPremAudit
        ...
        evidence: Object.entries({
          ...
          LockoutDurationInSeconds: row.LockoutDurationInSeconds,
          EnableBannedPasswordCheck: row.EnableBannedPasswordCheck,
          EnableBannedPasswordCheckOnPremises:
            row.EnableBannedPasswordCheckOnPremises,
          BannedPasswordCheckOnPremisesMode:
            row.BannedPasswordCheckOnPremisesMode,
