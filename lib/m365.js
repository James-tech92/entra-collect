...
async function collectM365CollabAndMail(graph, pool, io, findings, summary, opts = {}) {
  ...
  if (ctaDefault) {
    ...
    findings.push({
      ...
      Detail: `Cross-tenant default B2B inbound=${summary.ctaB2bCollaborationInbound}; outbound=${summary.ctaB2bCollaborationOutbound}`,
    });
  }
  ...
  if (ctaPartners) {
    ...
    io.saveCsv(
      ...
      ctaPartners.map((p) => ({
        ...
        IsInMyOrganization: p.isServiceProvider,
      }))
    );
  }
