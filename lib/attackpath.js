...
async function collectAttackPathChecks(graph, io, findings, summary, ctx = {}) {
  ...
  const secInfo = matchPolicies((p) =>
    hasUserAction(p, "registersecurityinfo")
  );
  caCoverage.push({
    Control: "Protect security-info registration (trusted location / compliant)",
    Covered: secInfo.length > 0,
    Policies: secInfo.map((p) => p.displayName).join(" | "),
    Severity: "High",
  ...
  if (!secInfo.length) {
    ...
    checklist.push(
      checklistRow(
        ...
        "Pass",
        "High",
        secInfo.map((p) => p.displayName).join(" | "),
        "MFA takeover"
      )
    );
  }
  ...
  // Report-only that should be enforced (device compliance)
  const reportOnlyDevice = policies.filter((p) => {
    if (p.state !== "enabledForReportingButNotEnforced") return false;
    const g = builtIn(p);
    return g.includes("compliantdevice") || g.includes("domainjoineddevice");
  });
  ...
  for (const row of spPriv.slice(0, 40)) {
    ...
    if (!sp) continue;
    const secrets = (sp.passwordCredentials || []).length;
    const keys = (sp.keyCredentials || []).length;
    spSecretPriv.push({
      ...
      Risk:
        secrets > 0
          ...
          : keys > 0
            ...
            : "Info — no creds listed on SP (check app registration)",
    });
    if (secrets > 0) {
      push(
        findings,
  ...
  // Apps with GA-path Graph permissions
  ...
  if (gaPath.length) {
    const uniq = [...new Set(gaPath.map((r) => r.SPNDisplayName))];
    checklist.push(
      checklistRow(
        "AP.App.PathToGA",
        ...
        "Fail",
        "High",
        `${gaPath.length} assignments / apps: ${uniq.slice(0, 12).join(" | ")}`,
    ...
    push(
      ...
      "High",
      "AppPrivEsc",
      `${uniq.length} apps with GA-path Graph permissions (${[...PRIV_TO_GA_PERMS].join(", ")}) — 40_Apps_Path_To_GA.csv`
    );
  } else {
    checklist.push(
      checklistRow(
