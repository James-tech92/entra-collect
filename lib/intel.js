...
`.trim(),
    ...
    if (res && res.results) {
      ...
      const high = alertRows.filter((r) =>
        /high|critical/i.test(String(r.Severity || ""))
      ).length;
      findings.push({
        Severity: high ? "High" : alertRows.length ? "Medium" : "Info",
        Area: "Alerts",
        Detail: `${alertRows.length} Defender alert group(s) in 30d (${high} High/Critical) — 36_Security_Alerts_30d.csv`,
      });
    }
...
| where (isnotnull(Crit) and Crit <= 1)
    or isnotempty(RiskLevel)
    or isnotempty(AssignedRoles)
    or isnotempty(PrivilegedEntraPimRoles)
    or isnotempty(BlastRadius)
...
| sort by CriticalityLevel asc, RiskScore desc
| take 300
`.trim(),
      `
IdentityInfo
| where isnotempty(AssignedRoles) or isnotempty(PrivilegedEntraPimRoles) or isnotempty(RiskLevel)
    ...
    if (idInfo && idInfo.results) {
      ...
      if (rows.length) {
        findings.push({
          ...
          Area: "IdentityInfo",
          Detail: `${rows.length} IdentityInfo critical/risk/role row(s)` +
            (risky ? ` (${risky} with RiskLevel)` : "") +
...
`.trim(),
    ...
    if (acct && acct.results) {
      ...
      const noMfa = rows.filter((r) => !String(r.EnrolledMfas || "").trim()).length;
      if (rows.length) {
        findings.push({
          Severity: noMfa ? "Medium" : "Info",
          Area: "IdentityAccountInfo",
          Detail: `${rows.length} IdentityAccountInfo privileged/eligible row(s)` +
            (noMfa ? ` (${noMfa} with empty EnrolledMfas)` : "") +
