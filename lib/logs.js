...
/**
 * Query Graph sign-in logs with pagination and a max cap (Reader tenants can be huge).
 */
async function getSignIns(graph, filter, { top = 999, maxPages = 20 } = {}) {
  const items = [];
  let url =
    `${graph.GRAPH}/auditLogs/signIns?$filter=${encodeURIComponent(filter)}` +
    `&$top=${Math.min(top, 999)}&$orderby=createdDateTime desc`;

  // Prefer beta for richer authenticationProtocol fields
  const betaUrl =
    `${graph.GRAPH_BETA}/auditLogs/signIns?$filter=${encodeURIComponent(filter)}` +
    `&$top=${Math.min(top, 999)}&$orderby=createdDateTime desc`;
...
function isDeviceCodeSignIn(s) {
  const proto = String(s.authenticationProtocol || "").toLowerCase();
  const incoming = String(s.incomingTokenType || "").toLowerCase();
  const client = String(s.clientAppUsed || "").toLowerCase();
  const details = JSON.stringify(s.authenticationDetails || []).toLowerCase();
  return (
    proto.includes("devicecode") ||
    proto.includes("device code") ||
    incoming.includes("devicecode") ||
    details.includes("devicecode") ||
    client.includes("device code")
  );
}

function isLegacyClient(s) {
  const c = String(s.clientAppUsed || "");
  return /Exchange ActiveSync|Other clients|IMAP|POP|Authenticated SMTP|MAPI|Offline Address Book|Exchange Web Services/i.test(
    c
  );
}
...
async function collectSignInAndAuditInsights(graph, io, findings, summary, opts = {}) {
  ...
  // ── Device code via Graph filters (try several) ───────────────────────
  for (const days of windows) {
    const since = isoDaysAgo(days);
    summary[`signInWindow${days}d`] = since;

    // Attempt 1: authenticationProtocol eq 'deviceCode'
    let deviceCode = await soft(
      `signIns_deviceCode_${days}d`,
      () =>
        getSignIns(
          graph,
          `createdDateTime ge ${since} and authenticationProtocol eq 'deviceCode'`,
          { maxPages: 15 }
        ),
      io
    );

    // Attempt 2: broader fetch + local filter if filter unsupported
    if (!deviceCode) {
      deviceCode = [];
    }

    // If filter returned empty, try hunting (better retention / fields) when schema allows
    if (!deviceCode.length && id.dialect) {
    ...
    // Graph filter path succeeded (or empty array)
    if (deviceCode && deviceCode.length >= 0) {
      // Optional: also scan a sample of recent sign-ins if protocol filter empty
      if (!deviceCode.length) {
        const sample = await soft(
          `signIns_sample_${days}d`,
          () =>
            getSignIns(graph, `createdDateTime ge ${since}`, {
              maxPages: 5,
              top: 200,
            }),
          io
        ...
        if (sample && sample.length) {
          deviceCode = sample.filter(isDeviceCodeSignIn);
          io.saveJson(`20_signins_sample_meta_${days}d.json`, {
            sampled: sample.length,
            deviceCodeMatchedLocally: deviceCode.length,
            note: "authenticationProtocol filter may be unsupported; local match on sample only — prefer hunting for full coverage when schema has identity tables",
            huntingDialect: id.dialect,
          });
        }
      }
      ...
      pushFinding(
        ...
        "DeviceCode",
        rows.length
          ? `${rows.length} device-code sign-ins / ${users.length} users in last ${days}d — 20_DeviceCode_SignIns_${days}d.csv`
          : `No device-code sign-ins found in last ${days}d (or logs/API unavailable)`
      );
    }
  }
  ...
  let legacy = await soft(
    ...
    async () => {
      const all = await getSignIns(graph, `createdDateTime ge ${since}`, {
        maxPages: 10,
        top: 200,
      });
      return (all || []).filter(isLegacyClient);
    },
    io
  );
  ...
  if (!legacy || !legacy.length) {
    ...
    legacyDigestSource = legacy
      ...
      .map((r) => ({ ...r, Events: 1 }));
    io.saveCsv(
      `21_LegacyAuth_Success_${primaryDays}d.csv`,
      legacyDigestSource
    );
    ...
    if (legacyDigestSource.length) {
      pushFinding(
        ...
        "High",
        "LegacyAuth",
        `${legacyDigestSource.length} legacy-auth sign-in events sampled in ${primaryDays}d — 21_LegacyAuth_Success_${primaryDays}d.csv`
      );
    }
  }
  ...
  if (huntFails && huntFails.results && huntFails.results.length) {
    ...
    const graphFails = await soft(
      ...
      () =>
        getSignIns(
          graph,
          `createdDateTime ge ${since} and status/errorCode ne 0`,
          { maxPages: 10, top: 200 }
        ),
      io
    );
  ...
  if (failRows && failRows.length) {
    ...
    if (hot.length) {
      pushFinding(
        ...
        "Medium",
        "FailedSignIns",
        `${hot.length} IPs with ≥50 failures in ${primaryDays}d — 22_FailedSignIns_ByIP_${primaryDays}d.csv`
      );
    }
  } else if (!id.dialect) {
  ...
  if (huntNoMfa && huntNoMfa.results) {
    ...
    if (huntNoMfa.results.length) {
      pushFinding(
        findings,
        "Medium",
        "SingleFactor",
        `${huntNoMfa.results.length} users with successful non-MFA sign-ins (hunting) — 23_SingleFactor_Success_${primaryDays}d.csv`
      );
    }
  }
...
module.exports = { collectSignInAndAuditInsights, getSignIns };
