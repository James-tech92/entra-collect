...
/** July 2026 Patch Tuesday (+ OOB where relevant). Update this map each month. */
const PATCH_TUESDAY = {
  asOf: "2026-07-14",
  label: "July 2026 Patch Tuesday",
  builds: {
    // major.minor prefix → { minRevision, kb, product }
...
function parseOsBuild(osVersionBuild) {
  // e.g. "10.0.26100.8875" or "26100.8875"
  const s = String(osVersionBuild || "");
  const m = s.match(/(?:10\.0\.)?(\d{5})\.(\d+)/);
  if (!m) return null;
  return { major: m[1], ubr: Number(m[2]), raw: s };
}

function patchStatus(buildInfo) {
  if (!buildInfo) return { status: "Unknown", product: "?", kbExpected: "?", lagUbr: null };
  const ref = PATCH_TUESDAY.builds[buildInfo.major];
  if (!ref) {
...
async function collectEndpointHunts(graph, io, findings, summary, opts = {}) {
  ...
  // The Patch Tuesday table is maintained by hand. Say so loudly once it ages,
  // otherwise a stale baseline silently under-reports how far behind endpoints are.
  const referenceAgeDays = Math.floor(
    (Date.now() - Date.parse(PATCH_TUESDAY.asOf)) / 86400000
  );
  const referenceStale = !Number.isFinite(referenceAgeDays) || referenceAgeDays > 45;
  if (referenceStale) {
    const msg =
      `Patch Tuesday reference is ${referenceAgeDays} days old (asOf ${PATCH_TUESDAY.asOf}) — ` +
      "update PATCH_TUESDAY in lib/endpoints.js; patch-lag figures under-report until then";
    console.warn(`  ⚠ ${msg}`);
    findings.push({ Severity: "Medium", Area: "Coverage", Detail: msg });
  }
  summary.patchTuesdayAsOf = PATCH_TUESDAY.asOf;
...
`.trim()
  ...
  if (osHunt && osHunt.results) {
    for (const r of osHunt.results) {
      const ver = r.OSVersion || r.OSBuild || "";
      const build = parseOsBuild(ver) || parseOsBuild(r.OSBuild);
      const patch = patchStatus(build);
      ...
      osRows.push({
        ...
        LagUBR: patch.lagUbr,
        PatchTuesdayAsOf: PATCH_TUESDAY.asOf,
  ...
  if (osRows.length) {
    ...
    pushFinding(
      ...
      behind ? "High" : "Info",
      "PatchTuesday",
      `${behind} devices behind ${PATCH_TUESDAY.label} (${PATCH_TUESDAY.asOf}) — 32_Behind_PatchTuesday.csv`
    );
  }
