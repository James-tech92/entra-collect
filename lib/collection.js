...
async function runCollection(pool, outDir, opts = {}) {
  ...
  if (opts.portalPage) {
    await soft("portal_hunt_attach", () => attachPortalHunt(opts.portalPage), io);
  ...
  if (roleDefs) {
    ...
    io.saveJson("03_role_definitions.json", roleDefs);
  }
  ...
  if (org) io.saveJson("01_organization.json", org);
  ...
  const permanent = await soft(
    "roleAssignments",
    () => graph.getAll(`${graph.GRAPH}/roleManagement/directory/roleAssignments`),
    io
  );
  const eligible = await soft(
    "roleEligibility",
    () =>
      graph.getAll(
        `${graph.GRAPH}/roleManagement/directory/roleEligibilitySchedules`
      ),
    io
  );

  async function resolvePrincipal(principalId) {
    if (!principalId) return {};
    try {
      return await graph.get(`${graph.GRAPH}/directoryObjects/${principalId}`);
    } catch {
      return { id: principalId };
    }
  }

  async function principalRow(a, assignmentType, pim) {
    const p = await resolvePrincipal(a.principalId);
    const type = ((p["@odata.type"] || "").split(".").pop() || "").replace(
      /^#/,
      ""
    );
    return {
      RoleName: roleDefMap[a.roleDefinitionId] || a.roleDefinitionId,
      PrincipalName: p.displayName || "",
      PrincipalType: type || "unknown",
      UPNOrAppId:
        p.userPrincipalName ||
        (p.appId ? `appId: ${p.appId}` : p.mail || ""),
  ...
  if (permanent) {
    for (const a of permanent) {
      roleRows.push(await principalRow(a, "Permanent (direct)", "No"));
    }
  }
  if (eligible) {
    for (const a of eligible) {
      roleRows.push(await principalRow(a, "PIM — Eligible", "Yes"));
    }
  }
  ...
  let privRows = roleRows.filter((r) => HIGH_PRIV.has(r.RoleName));
  const rolesCollected = permanent != null || eligible != null;
  ...
  if (!rolesCollected) {
    ...
    summary.privilegedAssignmentsTotal = roleRows.length;
    summary.privilegedHighValueAssignments = privRows.length;
    summary.globalAdminPermanent = privRows.filter(
      (r) =>
        r.RoleName === "Global Administrator" &&
        r.AssignmentType === "Permanent (direct)"
    ).length;
    summary.pimEligibleCount = roleRows.filter(
      (r) => r.AssignmentType === "PIM — Eligible"
    ).length;

    findings.push({
      Severity: summary.globalAdminPermanent > 5 ? "High" : "Info",
      Area: "Privileged",
      Detail: `${summary.globalAdminPermanent} permanent Global Administrators; ${privRows.length} high-value role assignments (see 03_PrivilegedAccounts_HighValue.csv)`,
    });
  }
  ...
  if (regDetails) {
    ...
    if (noMfa.length) {
      findings.push({
        Severity: summary.adminsWithoutMfa ? "High" : "Medium",
        Area: "MFA",
        Detail: `${noMfa.length} users without MFA registered (${summary.adminsWithoutMfa} admins) — 07_Users_Without_MFA.csv`,
      });
    }
    findings.push({
      Severity: "Info",
  ...
  // ── SPN dangerous perms (keep) ────────────────────────────────────────
  console.log("── SPN / apps");
  const dangerous = {
  ...
  const graphSp = await soft(
    "graphSP",
    () =>
      graph.getAll(
        `${graph.GRAPH}/servicePrincipals?$filter=appId eq '00000003-0000-0000-c000-000000000000'`
      ),
    io
  );
  const graphSpObj = Array.isArray(graphSp) ? graphSp[0] : null;
  const permMap = {};
  if (graphSpObj && graphSpObj.appRoles) {
    for (const r of graphSpObj.appRoles) permMap[r.id] = r.value;
  }
  const graphAssignments = graphSpObj
    ? await soft(
        "graphAppRoleAssignedTo",
        () =>
          graph.getAll(
            `${graph.GRAPH}/servicePrincipals/${graphSpObj.id}/appRoleAssignedTo`
          ),
        io
      )
    : null;
  let dangerousSpnRows = [];
  if (graphAssignments) {
    for (const a of graphAssignments) {
      const perm = permMap[a.appRoleId];
      const sev = dangerous[perm];
      if (!sev) continue;
      dangerousSpnRows.push({
        SPNDisplayName: a.principalDisplayName,
        PrincipalId: a.principalId,
        PrincipalType: a.principalType,
        Permission: perm,
        Severity: sev,
        ResourceApp: a.resourceDisplayName,
  ...
  const apps = await soft(
    ...
    () =>
      graph.getAll(
        `${graph.GRAPH}/applications?$select=id,appId,displayName,web,spa,passwordCredentials`
      ),
    io
  );
  if (apps) {
  ...
  await collectAttackPathChecks(graph, io, findings, summary, {
    ...
    sources: {
      ...
      authz: authzPol != null,
      authMethods: authMethods != null,
      privRoles: permanent != null,
      regDetails: regDetails != null,
      servicePrincipals: graphAssignments != null,
    },
  });
...
`.trim(),
  ...
  // Try to read diagnostic settings via Graph beta if exposed (often empty without ARM)
  const diag = await soft(
    "entraDiagnosticHint",
    async () => {
      // No direct Graph for subscription diagnostic settings without ARM token.
      // Attempt: reportSubscriptions or security providers
      return graph.getAll(
        `${graph.GRAPH}/security/securescorecontrolprofiles?$filter=id eq 'adminMFAV2'&$top=1`
      );
    },
    io
  );
  ...
  if (failedSteps.length) {
    // A 401/403 will fail identically on every re-run — telling the operator to
    // resume would waste their time. Only transient causes are worth retrying.
    const denied = failedSteps.filter(
      ([, s]) => s.httpStatus === 401 || s.httpStatus === 403
    );
    ...
    if (denied.length) {
      ...
      for (const [label, s] of denied) {
        console.log(`      · ${label}: HTTP ${s.httpStatus}`);
      }
