...
var require_tokens = __commonJS({
  "lib/tokens.js"(exports, module) {
    ...
    function scopeSet(payload) {
      const scp = String(payload.scp || "").split(/\s+/).filter(Boolean);
      const roles = Array.isArray(payload.roles) ? payload.roles : [];
      return /* @__PURE__ */ new Set([...scp, ...roles]);
    }
    function hasPolicyRead(payload) {
      const s = scopeSet(payload);
      return s.has("Policy.Read.All") || s.has("Policy.ReadWrite.All") || s.has("Policy.ReadWrite.ConditionalAccess") || s.has("Policy.Read.ConditionalAccess");
    }
    function scoreToken(payload) {
      ...
      if (/api\.security\.microsoft\.com|api\.securitycenter\.microsoft\.com|api-[a-z]{2}\.security\.microsoft\.com/i.test(
        aud
      )) {
        score += 90;
      }
    ...
    function isMtpAud(aud) {
      ...
      return a.includes("api.security.microsoft.com") || a.includes("api.securitycenter.microsoft.com") || /api-[a-z]{2}\.security\.microsoft\.com/.test(a);
    }
    ...
    var EXPIRY_SKEW_SEC = 120;
    function tokenExpiresAt(payload) {
      const exp = Number(payload && payload.exp);
      return Number.isFinite(exp) ? exp : null;
    }
    function isTokenExpired2(payload, skewSec = EXPIRY_SKEW_SEC) {
      const exp = tokenExpiresAt(payload);
      if (exp == null) return false;
      return exp - skewSec <= Math.floor(Date.now() / 1e3);
    }
    ...
    var TokenPool2 = class {
      ...
      lockTenant(tid, source = "lock") {
        ...
        if (this.expectedTid && this.expectedTid !== n) {
          throw new Error(
            `Tenant lock conflict: pool is locked to ${this.expectedTid} (${this.expectedTidSource}), refusing to switch to ${n} (${source}). Start a fresh collect for the other tenant.`
          );
        }
        if (!this.expectedTid) {
          this.expectedTid = n;
          this.expectedTidSource = source;
          console.log(`  \u{1F512} tenant locked: ${n} (via ${source})`);
        }
       ...
       */
      async refresh({ minIntervalMs = 2e4 } = {}) {
      ...
      pruneExpired() {
        ...
        for (const [key, entry] of this.byHash) {
          if (isTokenExpired2(entry.payload)) {
      ...
      _rejectMixed(payload, meta, tid) {
        ...
        if (Date.now() - this._mixedWarnAt > 5e3) {
          this._mixedWarnAt = Date.now();
          console.warn(
            `  \u26A0 rejected token from other tenant tid=${tid} (locked=${this.expectedTid})${rec.upn ? ` upn=${rec.upn}` : ""} via ${rec.source} \u2014 keeping pool single-tenant`
          );
        }
        return null;
      }
      add(token, meta = {}) {
        ...
        const aud = String(payload.aud || "");
        const okAud = aud.includes("graph.microsoft.com") || aud === "00000003-0000-0000-c000-000000000000" || aud.includes("outlook.office365.com") || aud.includes("outlook.office.com") || aud.includes("manage.office.com") || isMtpAud(aud);
        if (!okAud) return null;
        if (isTokenExpired2(payload)) return null;
        ...
        if (tid) {
          ...
          if (Date.now() - this._mixedWarnAt > 15e3) {
            this._mixedWarnAt = Date.now();
            console.warn(
        ...
        console.log(
          `  \xB7 captured token aud=${aud.slice(0, 50)}${src} (${describeToken(payload)})`
        );
        return entry;
      }
      list() {
        return this.listAll().filter(
          (e) => String(e.aud || "").includes("graph.microsoft.com") || e.aud === "00000003-0000-0000-c000-000000000000"
        );
      }
      listMtp() {
      ...
      bestForAudience(substr) {
        return this.listAll().find(
          (e) => String(e.aud || "").toLowerCase().includes(String(substr).toLowerCase())
        ) || null;
      }
      hasMtp() {
      ...
      assertSingleTenant() {
        ...
        if (tids.length > 1) {
          return {
            ok: false,
            tids,
            error: `Mixed-tenant token pool: found ${tids.length} tenant IDs (${tids.join(", ")}). Close other tenant tabs / use a dedicated browser profile, then re-run.`
          };
        }
        if (this.expectedTid && tids.length === 1 && tids[0] !== this.expectedTid) {
          return {
            ok: false,
            tids,
            error: `Token tenant ${tids[0]} does not match locked tenant ${this.expectedTid} (${this.expectedTidSource}).`
          };
        }
...
var require_scopes = __commonJS({
  "lib/scopes.js"(exports, module) {
    ...
    function evaluatePermissions2(payloads, { hasPortalSession = false } = {}) {
    ...
    function printPermissionMatrix(payloads, opts = {}) {
      const { rows, missing, viaPortal } = evaluatePermissions2(payloads, opts);
...
`
      ...
      for (const r of rows) {
        const detail = r.status === "ok" ? r.matched.join(", ") : r.status === "portal" ? "via browser portal session" : `needs one of: ${r.needed.join(" | ")}`;
        console.log(`  ${icon[r.status]} ${r.area.padEnd(32)} ${detail}`);
      }
      if (missing.length) {
        console.log(`
  ${missing.length} area(s) will be missing from the report:`);
        for (const r of missing) console.log(`    \xB7 ${r.area} \u2192 ${r.impact}`);
      }
      if (viaPortal.length) {
        console.log(
          `
  ${viaPortal.length} area(s) rely on the browser portal session staying open for the whole run.`
    ...
    module.exports = { REQUIRED_SCOPES, evaluatePermissions: evaluatePermissions2, printPermissionMatrix };
  }
});
...
// web/src/config.js
var CLIENT_ID2 = "REPLACE_WITH_YOUR_APP_REGISTRATION_CLIENT_ID";
var AUTHORITY = "https://login.microsoftonline.com/organizations";
var REDIRECT_URI2 = window.location.origin + window.location.pathname;

// web/src/msal-browser-auth.js
...
function assertConfigured() {
  if (!CLIENT_ID2 || CLIENT_ID2.startsWith("REPLACE_WITH_")) {
    throw new Error(
      "No app registration configured. Edit web/src/config.js with your own Application (client) ID \u2014 see docs/WEBAPP.md."
    );
  }
}
...
function buildAdminConsentUrl({ tenantId = "organizations" } = {}) {
  assertConfigured();
...
function renderConfigWarning() {
  if (CLIENT_ID2 && !CLIENT_ID2.startsWith("REPLACE_WITH_")) return;
  el("configWarning").hidden = false;
  el("btnLogin").disabled = true;
}
function renderIdentity(payload, account) {
  const upn = payload.upn || payload.unique_name || payload.preferred_username || account.username;
...
function renderPermissionMatrix(payload) {
  ...
  if (missing.length) {
    ...
    el("consentBox").innerHTML = "<p>" + missing.length + ' area(s) need admin consent on this tenant. A Global Administrator / Privileged Role Administrator opens this link <strong>once</strong> for the whole tenant \u2014 after that, any Reader-level account (including this one) gets these scopes on sign-in:</p><a class="btn" target="_blank" rel="noopener" href="' + escapeHtml(buildAdminConsentUrl({ tenantId: tid })) + '">Grant admin consent \u2197</a>';
...
async function handleSignedIn(account) {
  ...
  const payload = (0, import_tokens.decodeJwt)(result.accessToken);
  if (!payload) {
...
@azure/msal-common/dist-browser/crypto/DpopProofGenerator.mjs:
